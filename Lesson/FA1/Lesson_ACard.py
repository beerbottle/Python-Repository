...
if __name__ == '__main__':
    ...
    # 缺失值填充
    allNoMissDate = u_modify.makeup_miss_for_num(p_df=data_all, p_var_list=number_var_list, p_method='perc50')
    u_explore.num_var_perf(p_df=allNoMissDate, p_var_list=number_var_list, p_target_var=col_target
                           , p_path=path_explore_result + '\\1')
    allNoMissDate = u_modify.makeup_miss_for_str(p_df=allNoMissDate, p_str_var_list=string_var_list
                                                 , p_method='random')
    u_explore.str_var_pref(p_df=allNoMissDate, p_var_list=string_var_list, p_target_var=col_target
                           , p_path=path_explore_result + '\\1')
